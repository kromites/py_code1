...
def check_events(ai_settings, screen, ship, stats, sb, play_button, aliens, bullets):
    """响应鼠标和键盘event"""
...
def fire_bullet(ai_settings, screen, ship, bullets):
    if len(bullets) < ai_settings.bullets_allowed:
        new_bullet = Bullet(ai_settings, screen, ship)  # 创建一个子弹，将其加入group bullets中
...
def updated_bullets(ai_settings, screen, stats, sb, ship, aliens, bullets):
    """更新子弹的位置并删除消失的子弹"""
    # 更新子弹的位置
    bullets.update()
    # 删除已经消失的子弹
...
def get_number_row(ai_settings, ship_height, alien_height):
    ...
    number_row = int(avaliable_space_y / (2 * alien_height))
    return number_row
